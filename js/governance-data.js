...
export const VOTE_STATUSES = Object.freeze(["open", "closing", "closed"]);
...
export async function listMeetingAgenda(meetingId, profile) {
  ...
  return snapshot.docs.map((entry) => ({ id: entry.id, ...entry.data() })).sort((a, b) => (Number(a.order) || 0) - (Number(b.order) || 0));
}
...
export async function listMeetingVotes(meetingId, profile) {
  ...
  return snapshot.docs.map((entry) => ({ id: entry.id, ...entry.data() })).sort((a, b) => timestampValue(b.openedAt) - timestampValue(a.openedAt));
}
...
export async function listResolutions(profile) {
  ...
  return snapshot.docs.map((entry) => ({ id: entry.id, ...entry.data() })).sort((a, b) => timestampValue(b.createdAt) - timestampValue(a.createdAt));
}
...
export async function createAgendaItem(meetingId, input, profile) {
  ...
  if (["adjourned", "cancelled"].includes(meeting.status) || meeting.recordStatus === "certified") throw new Error("The agenda is locked for this meeting.");
  ...
  await runTransaction(db, async (transaction) => {
    const freshMeeting = (await transaction.get(doc(db, "meetings", meetingId))).data();
    if (!freshMeeting || ["adjourned", "cancelled"].includes(freshMeeting.status) || freshMeeting.recordStatus === "certified") throw new Error("The agenda is locked for this meeting.");
    if (documentId) {
      const documentRef = doc(db, "documents", documentId);
      const freshDocument = (await transaction.get(documentRef)).data();
      if (!freshDocument || freshDocument.status !== "agenda_ready" || (freshDocument.agendaMeetingId && freshDocument.agendaMeetingId !== meetingId)) {
        throw new Error("That Board document is no longer available for this agenda.");
      }
      transaction.update(documentRef, { agendaMeetingId: meetingId, updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
    }
    transaction.set(itemRef, {
      agendaNumber: agendaNumber(itemRef.id),
      meetingId,
      meetingNumber: freshMeeting.meetingNumber || null,
      order,
      itemType,
      title,
...
export async function setAgendaItemStatus(agendaItemId, nextStatus, profile) {
  ...
  if (!meetingSnapshot.exists() || ["adjourned", "cancelled"].includes(meetingSnapshot.data().status)) throw new Error("The agenda is locked for this meeting.");
  await runTransaction(db, async (transaction) => {
    const fresh = await transaction.get(itemRef);
    if (!fresh.exists()) throw new Error("Agenda item not found.");
    transaction.update(itemRef, { status: nextStatus, updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
  });
}
...
export async function createMotion(meetingId, agendaItemId, text, profile) {
  ...
  await runTransaction(db, async (transaction) => {
    const [freshMeeting, freshAttendance, freshAgenda] = await Promise.all([
      transaction.get(doc(db, "meetings", meetingId)),
      transaction.get(doc(db, "meetingAttendance", `${meetingId}_${auth.currentUser.uid}`)),
    ...
    transaction.update(doc(db, "agendaItems", agendaItemId), { status: "active", updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
  });
...
export async function secondMotion(motionId, profile) {
  ...
  await runTransaction(db, async (transaction) => {
    const motionSnapshot = await transaction.get(motionRef);
    ...
    const [meetingSnapshot, attendanceSnapshot] = await Promise.all([
      transaction.get(doc(db, "meetings", motion.meetingId)),
    ...
    transaction.update(motionRef, {
      ...
      secondedAt: serverTimestamp(), updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid
    });
  });
}
...
export async function openVote(input, profile) {
  ...
  if (meeting.activeVoteId) throw new Error("Another Board vote is already open for this meeting.");
  ...
  await runTransaction(db, async (transaction) => {
    const [freshMeetingSnapshot, freshMotionSnapshot] = await Promise.all([
      transaction.get(doc(db, "meetings", motion.meetingId)),
      transaction.get(doc(db, "motions", motionId))
    ]);
    ...
    transaction.set(voteRef, {
      ...
      meetingNumber: freshMeetingSnapshot.data().meetingNumber || null,
      agendaItemId: motion.agendaItemId, motionId,
      question: String(input.question || motion.motionText).trim() || motion.motionText,
      status: "open", ballotVisibility, thresholdMode, eligibleVoterUids,
      recusedDirectorUids, quorumSnapshotPresent: quorum.presentEligible,
      quorumSnapshotRequired: quorum.required, openedBy: auth.currentUser.uid,
      openedByName: profile.displayName || profile.fullName || "Vote Controller",
      openedAt: serverTimestamp(), closingBy: null, closingByName: null, closingAt: null,
      closedBy: null, closedByName: null, closedAt: null,
      approveCount: null, opposeCount: null, abstainCount: null, ballotCount: null,
      requiredApproveCount: null, result: "pending", resolutionId: null,
      createdAt: serverTimestamp(), updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid
    });
    recusedDirectorUids.forEach((uid) => {
      const attendanceRecord = presentEligible.find((entry) => entry.directorUid === uid);
      transaction.set(doc(db, "voteRecusals", recusalId(voteRef.id, uid)), {
        voteId: voteRef.id, meetingId: motion.meetingId, directorUid: uid,
        directorName: attendanceRecord?.directorName || "Director",
        reason: String(input.recusalReason || "").trim() || null,
        recordedBy: auth.currentUser.uid, recordedAt: serverTimestamp()
      });
    });
    transaction.update(doc(db, "meetings", motion.meetingId), { activeVoteId: voteRef.id, updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
    transaction.update(doc(db, "motions", motionId), { status: "voting", voteId: voteRef.id, updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
    transaction.update(doc(db, "agendaItems", motion.agendaItemId), { status: "active", updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
  });
...
export async function castVote(voteId, choice, profile) {
  ...
  await runTransaction(db, async (transaction) => {
    const [freshVote, freshAttendance, existingBallot] = await Promise.all([
      transaction.get(doc(db, "votes", voteId)),
      transaction.get(doc(db, "meetingAttendance", `${vote.meetingId}_${auth.currentUser.uid}`)),
    ...
    transaction.set(ballotRef, {
      ...
      choice, ballotVisibility: freshVote.data().ballotVisibility, submittedAt: serverTimestamp()
    });
  });
}

async function readFrozenBallots(vote) {
  const snapshots = await Promise.all((vote.eligibleVoterUids || []).map((uid) => getDoc(doc(db, "voteBallots", ballotId(vote.id, uid)))));
...
export async function closeVote(voteId, profile) {
  ...
  let voteSnapshot = await getDoc(voteRef);
  if (!voteSnapshot.exists()) throw new Error("Vote not found.");
  let vote = { id: voteSnapshot.id, ...voteSnapshot.data() };
  if (vote.status === "closed") throw new Error("This vote is already closed.");

  const [meetingSnapshot, attendanceSnapshot] = await Promise.all([
    getDoc(doc(db, "meetings", vote.meetingId)),
    getDocs(query(collection(db, "meetingAttendance"), where("meetingId", "==", vote.meetingId)))
  ]);
  ...
  if (meetingSnapshot.data().activeVoteId !== voteId) throw new Error("This vote is not the meeting's active ballot.");
  const quorum = calculateQuorum(meetingSnapshot.data(), attendanceSnapshot.docs.map((entry) => entry.data()));
  if (!quorum.achieved) throw new Error(`Quorum is no longer present (${quorum.presentEligible}/${quorum.required}). The vote remains open.`);

  if (vote.status === "open") {
    await runTransaction(db, async (transaction) => {
      const [freshVoteSnapshot, freshMeetingSnapshot] = await Promise.all([
        transaction.get(voteRef),
        transaction.get(doc(db, "meetings", vote.meetingId))
      ]);
      if (!freshVoteSnapshot.exists() || freshVoteSnapshot.data().status !== "open") return;
      if (!freshMeetingSnapshot.exists() || freshMeetingSnapshot.data().status !== "in_session" || freshMeetingSnapshot.data().activeVoteId !== voteId) throw new Error("The vote can no longer enter closing state.");
      transaction.update(voteRef, {
        status: "closing", closingBy: auth.currentUser.uid,
        closingByName: profile.displayName || profile.fullName || "Vote Controller",
        closingAt: serverTimestamp(), updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid
      });
    });
    voteSnapshot = await getDoc(voteRef);
    vote = { id: voteSnapshot.id, ...voteSnapshot.data() };
  }
  if (vote.status !== "closing") throw new Error("The vote is not ready to finalize.");

  const ballots = await readFrozenBallots(vote);
  const counts = {
  ...
  await runTransaction(db, async (transaction) => {
    const [freshVoteSnapshot, freshMeetingSnapshot, motionSnapshot] = await Promise.all([
      transaction.get(voteRef),
      transaction.get(doc(db, "meetings", vote.meetingId)),
      transaction.get(doc(db, "motions", vote.motionId))
    ]);
    if (!freshVoteSnapshot.exists() || freshVoteSnapshot.data().status !== "closing") throw new Error("The vote is no longer in closing state.");
    if (!freshMeetingSnapshot.exists() || freshMeetingSnapshot.data().status !== "in_session" || freshMeetingSnapshot.data().activeVoteId !== voteId) throw new Error("The meeting no longer owns this active vote.");
    if (!motionSnapshot.exists() || motionSnapshot.data().status !== "voting") throw new Error("The motion record is not ready to finalize.");
    const motion = motionSnapshot.data();
    const meeting = freshMeetingSnapshot.data();

    transaction.update(voteRef, {
      status: "closed", approveCount: result.approve, opposeCount: result.oppose,
      abstainCount: result.abstain, ballotCount: ballots.length,
      requiredApproveCount: result.required, result: result.result,
      resolutionId: resolutionRef.id, closedBy: auth.currentUser.uid,
      closedByName: profile.displayName || profile.fullName || "Vote Controller",
      closedAt: serverTimestamp(), updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid
    });
    transaction.update(doc(db, "meetings", vote.meetingId), { activeVoteId: null, updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
    transaction.update(doc(db, "motions", vote.motionId), {
      status: result.adopted ? "adopted" : "failed", resolutionId: resolutionRef.id,
      updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid
    });
    transaction.update(doc(db, "agendaItems", vote.agendaItemId), { status: "completed", updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
    transaction.set(resolutionRef, {
      resolutionNumber: resolutionNumber(resolutionRef.id), meetingId: vote.meetingId,
      meetingNumber: meeting.meetingNumber || vote.meetingNumber || null,
      agendaItemId: vote.agendaItemId, motionId: vote.motionId, voteId,
      title: vote.question, resolutionText: motion.motionText, status: result.result,
      thresholdMode: vote.thresholdMode, ballotVisibility: vote.ballotVisibility,
      approveCount: result.approve, opposeCount: result.oppose, abstainCount: result.abstain,
      ballotCount: ballots.length, eligibleVoterCount: vote.eligibleVoterUids?.length || 0,
      requiredApproveCount: result.required, movedByUid: motion.movedByUid,
      movedByName: motion.movedByName, secondedByUid: motion.secondedByUid,
      secondedByName: motion.secondedByName, adoptedAt: result.adopted ? serverTimestamp() : null,
      createdAt: serverTimestamp(), createdBy: auth.currentUser.uid,
      certified: false, certifiedAt: null, certifiedBy: null, certifiedByName: null,
      recordId: null
    });
  });
